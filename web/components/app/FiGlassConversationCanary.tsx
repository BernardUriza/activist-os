...
/**
 * FiGlassConversationCanary — renders the Activist OS workflow through the
 * canonical fi-glass AgentConversationSurface instead of the bespoke dashboard
 * cards. Opt-in only (/app?fi_glass_canary=1); the default dashboard is
 * untouched. Proves the multi-agent handoff transcript (8 messages, the
 * CAMPAIGN⇄SAFETY veto loop) maps cleanly onto fi-glass's chat surface, with
 * per-message agent identity and VETO/APPROVED severity in the slot system.
 */
...
export function FiGlassConversationCanary({ runId }: { runId?: string | null }) {
  ...
  // Controlled / external-transcript mode (fi-glass >=1.2.0): the workflow owns
  // the thread (1 send -> 8+ handoff messages), the canonical hook supplies
  // send/turn/error/retry machinery. No hand-rolled AgentConversation.
