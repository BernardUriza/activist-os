"""
Transport-parity contract — the canonical invariant lifted from the old
standalone's ``test_transport_contract.py`` (the test IS the spec), re-authored
against the fresh canonical build. See ``docs/CANONICAL_CONTRACT.md`` §1–2 and
``docs/REUSE_MAP.md`` (PRESERVE bucket).

The contract every transport must satisfy for one demo run:
  - the run COMPLETES
  - the ``from_agent`` order is exactly:
        evidence, campaign, safety, campaign, safety, outreach, coordinator, reporter
  - at least one ``SAFETY_VETO`` (at index 2), followed by ``SAFETY_APPROVED``
    (at index 4) — one veto → one revision → approval (the money shot)
  - LocalTransport and BandTransport produce an EQUIVALENT coordination history
  - in Band mode: 4 real agents recruited, room stays at the 5-seat cap,
    ``reporter`` and ``system`` are virtual, every handoff carries the run's
    ``band_room_id`` (one room per run)
...
RED until Step 3 ports ``app.runner`` + ``app.models`` onto the canonical base.
"""
...
RECRUITED = [n for n in AGENT_NAMES if n not in ("evidence", "reporter")]
...
def assert_coordination_contract(result, handoffs):
    ...
    assert HandoffType.SAFETY_VETO in types, "history must contain a Safety VETO"
    veto_idx = types.index(HandoffType.SAFETY_VETO)
    assert veto_idx == VETO_INDEX, f"veto_index must be {VETO_INDEX}, got {veto_idx}"
    assert HandoffType.SAFETY_APPROVED in types[veto_idx + 1:], (
        "Safety APPROVED must come strictly after the veto"
    )
    assert types.index(HandoffType.SAFETY_APPROVED) == APPROVED_INDEX, (
        f"approved_index must be {APPROVED_INDEX}"
    )
...
class FakeBandClient:
    """Mimics the AsyncRestClient surface BandTransport touches — no network."""
...
async def test_band_publishes_coordination_history(fake_band):
    ...
    assert len(store.rooms) == 1, "exactly one Band room per run"
    room_id = store.rooms[0][1]
    assert all(h.metadata.band_room_id == room_id for h in handoffs)

    assert {p[2] for p in store.participants} == {f"id-{n}" for n in RECRUITED}, (
        "4 agents recruited — evidence initiates, reporter is virtual"
    )
...
async def test_band_virtual_reporter_not_added_as_participant(fake_band):
    transport, store = fake_band
    _, handoffs = await run_workflow(transport)

    assert all(p[2] != "id-reporter" for p in store.participants), (
        "virtual reporter must never occupy a Band room seat"
    )
...
# The opt-in real-Band smoke lives in test_real_band_smoke.py.
